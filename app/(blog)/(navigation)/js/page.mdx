...
자바스크립트 공부 기록입니다. 공부하며 기억할만한 ~~얼척없던~~ 것들을 이곳에
기록해요.
...
문자열 비교는 사실상 16비트 정수 배열의 비교와 같아서 통념과 결과값이 다를 수
있습니다:
...
덜 최적화합니다. 그렇다고 alias된 이름으로 eval을 호출하는 경우까지 알 수는
없으므로 이 경우 eval은 전역 스코프에서 동작합니다. 이를 eval의 **indirect
call**이라 합니다. 이는 코드를 독립된 top-level 스크립트로 실행시킬 때도
유용합니다.
...
delete 연산자는 프로퍼티가 삭제되었으면 true를, 없었으면 false를 반환합니다.
...
for/in은 객체면 다 됩니다. 프로퍼티 이름이 심볼이면 안되고 문자열이면
enumerable한 것만 순회합니다.

for/in은 상속받은 프로퍼티도 순회해서 for/in대신 for/of과 Object.keys()를 쓰는
경우가 많습니다.
