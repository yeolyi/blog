...
필요시 위 표에 따라 문자열로 변환합니다. 숫자도 마찬가지로 prefer-number를
돌리고 숫자로 변환합니다.
...
prefer 어쩌구 알고리즘을 이해하려면 두 메서드를 알아야합니다. 모든 객체가 이
메서드들을 상속받습니다.
...
- prefer-string: toString이 **원시값을 반환한다면** 이를 사용, 아니면 valueOf가
  원시값 반환하면 이를 사용, 아니면 TypeError.
