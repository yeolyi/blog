...
정의됩니다. 벤더들의 문서는 JS 개발자보다는 API를 개발하는 C++ 개발자 중심으로
작성되어있고, MDN web docs에는 사용자 입장으로 작성되어있습니다.
...
하지만 이는 웹페이지의 파싱과 렌더링을 크게 늦출 수 있습니다. 대신 스크립트를
`defer` 혹은 `async`하게 실행할 수 있습니다. 이들은 스크립트가
`document.write()`를 하지 않음을 암시하기에 브라우저는 스크립트를 다운로드하는
동시에 document를 파싱하고 렌더링할 수 있습니다.

![defer vs async](/deferasync.png)

`defer`는 document의 로드가 완료될 때까지 스크립트 실행을 미루고 `async`는
가능한 빨리 스크립트를 실행합니다. `defer`는 스크립트 선언 순서대로 실행되고
`async`는 순서가 보장되지 않습니다. 모듈 스크립트는 기본적으로 `defer`입니다.

여담으로 모듈이 아닌 인라인 스크립트에서는 defer와 async가 무시됩니다. 아래
예제에서 `type="module"`을 `defer`로 바꾸면 효과가 없을거에요:
...
있고 텍스트마다 상응되는 Text 객체가 있습니다. 두 객체는 다시 Node 클래스를
부모로 가집니다.
...
Client-side JS에서 **프로그램**이 무엇인지 공식적으로 정의된 바는 없지만,
document에 포함되거나 document가 참조하는 자바스크립트 코드들로 구성된다고 할 수
있습니다. 이들은 모두 공통의 전역 Window 객체를 공유하며 HTML document를
나타내는 동일한 DOM 객체에 접근 가능합니다.

다만 `iframe`은 독립된 전역 Window 객체와 DOM 객체를 가집니다.
...
1. 파서가 `async`한 스크립트를 만나면 비동기적으로 다운로드 후 즉시 실행합니다.
   모듈이면 의존성을 가진 다른 스크립트들도 모두 다운로드합니다.
1. 파싱이 완료됩니다. `document.readyState == "interactive"`.
...
navigator 프로퍼티는 브라우저와 OS에 대한 정보를 줘요:
...
발생한 에러가 처리되지 않더라도 등록된 이벤트 핸들러들은 잘 실행됩니다.
