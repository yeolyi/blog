...
각 함수 호출은 인자 외에도 **invocation context**라는 값을 가지고 이는 `this`
키워드의 값입니다. 함수가 객체를 통해(객체의 메서드로서) 호출되면 해당 객체가
invocation context가 됩니다.

따라서 `this`를 암묵적으로 전달된 함수 인자로 볼 수도 있어요. 이런 관점이 보다
