...
`Array.form`은 이터러블이나 유사 배열을 배열로 반환하는 팩토리 메서드로 스프레드
연산자와 유사하게 동작해요. **Web API에서 종종 유사배열을 반환하기에 이 때
유용하게 사용돼요**. 두번쨰 인자로 함수를 건네줄 수 있는데 상황에 따라 배열의
생성과 매핑을 한번에 할 수 있어 map보다 효율적입니다.
...
> are integers between 0 and 2^32-2 are indexes
...
희소 배열이 희소(sparse)할수록 더 느리고 메모리 효율적인 방식으로 처리됩니다.

사실 실전에서 대부분의 배열은 희소하지 않을겁니다 ㅎㅎ;

### 4. Array Length

배열 요소의 인덱스는 항상 length값 미만입니다:
...
배열에 값을 push하는 것과 a[a.length]에 값을 할당하는 것은 동일합니다.
...
a.forEach(
  ...
  // 다른 대부분의 메서드들도 여기 값이 함수 내부의 this가 됩니다.
  { val: 'hello' },
);
```
...
reduce 함수는 앞에서 본 세 인자들 전에 지금까지의 누적된 결과를 인자로
전달합니다. 함수에 initial value를 전달해주지 않으면 첫번째 값을 사용합니다:
...
reduce/reduceRight는 this를 설정하는 인자가 없습니다. 설정해야된다면 bind를 쓰면
돼요.
...
`a.flatMap(f)`는 `a.map(f).flat()`과 같지만 더 효율적입니다. `flatMap`을 입력
배열의 각 요소에 여러 개(혹은 0개)의 요소를 대응시킬 수 있도록 일반화된
`map`으로 생각할 수 있어요:
...
기존 배열을 바꾸지 않아요. `a = a.concat(x)` 보다는 push나 splice 메서드를
쓰는게 낫습니다.
