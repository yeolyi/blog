...
> When a function is invoked on or through an object, that object is the
> invocation context or this value for the function.
...
화살표함수는 정의된 환경의 this 값을 그대로 사용합니다. 화살표함수는 `prototype`
프로퍼티를 가지지 않아 클래스 생성자로서 사용될 수 없습니다.

### 2. Invoking Functions

함수는 다음 방법으로 호출될 수 있어요:

- As functions
- As methods
- As constructors
- Indirectly through their call() and apply() methods
- Implicitly, via JavaScript language features that do not appear like normal
  function invocations

strict mode면 this가 undefined, 아니면 전역 객체가 됩니다:
...
메서드는 객체의 프로퍼티에 할당된 함수입니다. 객체는 **invocation context**가
되고 함수 본문에서 **this** 키워드로 접근할 수 있습니다. 이 this값을 암묵적인
함수 인자로 볼 수도 있어요.

함수가 this를 반환하도록하면 **method chaining** 스타일의 API를 만들 수 있어요:

```
new Square().x(100).y(100).size(50).outline('red').draw();
```

중첩 함수의 this는 외부 스코프의 this값과 다를 수 있어요. JS의 결함으로
여겨진다고 합니다:
...
new 키워드를 붙이면 타겟 함수는 **생성자(constructor)**로 기능하고 일반 함수
호출과는 다르게 처리됩니다. 생성자의 `prototype` 프로퍼티를 상속받는 객체가
생성되고 함수 본문의 `this`가 이 객체를 가르킵니다. 함수 본문의 return에서
객체를 반환한다면 그 객체가 반환되고, 나머지 경우는 무시됩니다.

get/set, toString, valueOf, 순회 관련 함수들, tagged template literal,
Proxy등에서 implicit하게 함수가 호출될 수 있습니다.
...
위 코드에서 인자의 기본값은 호출마다 초기화되고 이는 파이썬과의 차이점입니다:

https://twitter.com/holdenmatt/status/1774866242282672365

기본값이 상수일 필요는 없고 다른 인자값에 의존적인 것도 가능합니다:

```js
let rec = (width, height = width * 2) => ({ width, height });
console.log(rec(1));
```
...
function max(first = -Infinity, ...rest) {
  // 여기서 rest는 대응되는 인자가 없을시 빈 배열이지, undefined이지는 않습니다.
  let maxVal = first;
...
console.log([...Array(100).keys()].map(fibo).join(' '));
```
