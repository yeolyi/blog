...
콜백은 아래처럼 중첩되어 callback hell을 만들어내지만 프로미스 체인은
선형입니다.

<ImageViewer src={callbackhell} alt="콜백 지옥 이미지" widthPercentage="80%" />

또한 콜백은 에러 처리가 어렵습니다. 비동기 코드에서 에러가 던져져도 호출자로
...
실습을 위해 `wait(ms)`을 정의했어요. 구현은 보지 마시고 ms후에 완료되는 작업을
표현한 비동기 함수라고 생각해주세요.
...
let callback = () => console.log('👍');
f().then(callback); // f가 완료되었지만 콜백이 여기서 실행되지는 않습니다.
