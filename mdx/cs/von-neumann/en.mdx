...
> One of the peculiarities that distinguished ENIAC from all subsequent computers was the way instructions were set into the machine. It was similar to the plugboard of a small punched-card machine, but ENIAC had about 40 plugboards, each several feet in size, and setting up a problem required many wires for each instruction.
To prepare a problem for execution required connecting thousands of wires, a task that took several days, and verifying the setup required many more days.
> 
