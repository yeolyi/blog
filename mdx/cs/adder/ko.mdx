...
[사실 엄마 아빠가 둘 다 좋지는 않았던 컴퓨터](/cs/nand-is-all-you-need)의 내용을 참고해 위 회로를 NAND로 구현하면 아래와 같습니다. 위쪽 출력이 Carry, 아래쪽 출력이 Sum입니다.
...
전가산기는 반가산기와 OR 게이트를 활용해 만들 수 있습니다. 아래 예제에서는 OR 게이트의 출력이 즉 Carry이고, 오른쪽 반가산기의 SUM 출력이 최종 SUM이 됩니다.
...
전가산기 10개면 1000이 넘는, 30개면 10억이 넘는 수까지의 덧셈도 가능합니다!
