...
- 크롬에서 call stack이 몇 개밖에 안보인다면 show ignore-listed frames 옵션을 체크했는지 확인하자 🥲
...
크롬 inspector에서 DOM 요소 우클릭을 하면 DOM이 바뀔 때 breakpoint를 설정할 수 있다. `<div id="root"></div>` 에 breakpoint를 걸고 스크립트 실행을 재개해보자. 
...
지금까지 사진을 보면 모두 `performWorkUntilDeadline`에서 시작함이 인상깊다. 
...
이제 이 과정이 어떻게 구현되어있는지 간단한 `<App/>`이 렌더되는 과정을 살펴보며 배워보자.
