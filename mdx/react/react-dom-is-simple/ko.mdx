...
ReactDOM의 `createRoot`만 써도 단순한 리액트 앱을 렌더하는데 문제 없다.
...
`ReactDomRoot.js`에서 import한걸 그대로 reexport한다. 간략화시킨 `ReactDOMRoot.js`를 살펴보자.
...
// [!code highlight:3]
// 최종적으로 updateContainer를 호출한다.
// 이전에 만든 createRoot의 반환값과 우리가 render에 넘긴 컴포넌트를 인자로 넘긴다.
...
관련 코드는 react-dom-bindings 패키지에 있다. 살펴보면 아래처럼 `createElement`같은 DOM 함수들이 드문드문 보인다. 
...
// 샌박 환경이라그런가 이상하게 나와서 constructor.name으로 했다.
